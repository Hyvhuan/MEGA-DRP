...
for m in range(5):
    ...
    lc = LayerConductance(model, model.pm.pm.cd_attn)
    attrs = []
    ...
    for batch in tqdm(interpret_loader):
        ...
        lc_attr = lc.attribute((exp, mut, cnv), additional_forward_args=(gene_indexes, adj_mat, v_feature), target=0)

        for b in range(batch_size):
            attr = (b_response_id[b], b_cell_id[b], b_drug_id[b], lc_attr[b].detach().cpu().numpy(),)
            attrs.append(attr)

    with open(f"layer_attr_m{m}.pkl", "wb") as f:
        pickle.dump(attrs, f)