...
predict_set = MEGA_DRP_Dataset("test_pdx")
...
for m in range(5):
    ...
    with open(f"../AUC_PDX/auc_values_model_{m}.txt", "w") as f:
        for drug_id, auc_list in auc_values.items():
            f.write(f"Drug ID {drug_id}: {','.join(map(str, auc_list))}\n")

    # fig, axes = plt.subplots(1, 4, figsize=(16, 2.25), sharey="all")
    # count = 0
    #
    # for drug_id, drug, _ in drugs:
    #
    #     axes[count].plot(epoch, auc_values[drug_id], marker="o", color="b", label="AUC Trend")
    #     axes[count].set_xlabel("Epoch")
    #     axes[count].set_ylabel("AUC")
    #     axes[count].set_ylim(0, 1)
    #     axes[count].set_title(f"{drug}(model {m})", fontweight="bold")
    #     axes[count].legend(loc="lower right")
    #     count += 1
    #
    # plt.tight_layout()
    # plt.savefig(f"../AUC_PDX/auc_trends_model_{m}.png", format="png")
    # plt.close()
